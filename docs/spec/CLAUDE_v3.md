...
Tech Stack:       [e.g., Python 3.11 · NumPy · Flask · PostgreSQL]
Primary Language: [e.g., Python]
Hypothesis Type:  [Performance / Correctness / Debt / Architecture]
Forge Session:    [Session number, e.g., Session 3]
Repository Root:  [e.g., ~/projects/kibo/]
```
...
│     Query Obsidian: /Forge/Projects/[NAME]/ (project-specific)      │
...
1. **Never modify `EVAL.sh` or `EVAL_SPEC.md`.** These are write-protected by architecture.
...
| New — seen only once | Add to project-local `/Forge/Projects/[NAME]/` only |
...
|---|---|
| Current project context | `/Forge/Projects/[THIS PROJECT NAME]/` only |
...
Step 5: Query Obsidian /Forge/Projects/[NAME]/ — load project context
...
═══════════════════════════════════════════
  ...
  Win rate (all):    XX%
  Cycles completed:  N
  Patterns loaded:   N from /Patterns/, N from /Projects/[NAME]/
